package owo.bladecraftstudios.toomanyfountains.core.items;
...
import owo.bladecraftstudios.toomanyfountains.server.blocks.DWCircusBlocks;
...
public class SilwareBucketItem extends BucketItem {
    private final Fluid content;

    public SilwareBucketItem(Fluid pContent, Properties pProperties) {
        ...
        if (ret != null) {
            ...
            if (pLevel.mayInteract(pPlayer, blockpos) && pPlayer.mayUseItemAt(blockpos1, direction, itemstack)) {
                if (this.content == Fluids.EMPTY) {
                    ...
                    BlockPos blockpos2 = this.canBlockContainFluid(pLevel, blockpos, blockstate) ? blockpos : blockpos1;
    ...
    public static ItemStack getEmptySuccessItem(ItemStack pBucketStack, Player pPlayer) {
        return !pPlayer.getAbilities().instabuild ? new ItemStack(DWItems.SILWARE_BUCKET.get()) : pBucketStack;
    }
}
